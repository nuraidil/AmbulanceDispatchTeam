...
public class AmbulanceDispatch_Djikstra {
    ...
    static class NodeRecord implements Comparable<NodeRecord> {
        ...
        NodeRecord(int nodeId, double costSoFar) {
            this.nodeId = nodeId;
        ...
        public int compareTo(NodeRecord other) {
            return Double.compare(this.costSoFar, other.costSoFar);
        }
    }
    ...
    public static AStarResult dijkstraSearch(Graph graph, int startId, int goalId) {
        Map<Integer, Double> dist = new HashMap<>();
        Map<Integer, Integer> cameFrom = new HashMap<>();
        Set<Integer> closedSet = new HashSet<>();

        if (!graph.nodes.containsKey(goalId)) {
        ...
        openSet.add(new NodeRecord(startId, 0.0));
        ...
        while (!openSet.isEmpty()) {
            ...
            if (current == goalId) {
                ...
                return new AStarResult(dist.get(current), path, nodesExpanded);
            }
            ...
            for (Edge edge : graph.adj.getOrDefault(current, Collections.emptyList())) {
                ...
                double tentativeDist = dist.getOrDefault(current, Double.MAX_VALUE) + edge.cost;

                if (tentativeDist < dist.getOrDefault(neighbor, Double.MAX_VALUE)) {
                    cameFrom.put(neighbor, current);
                    dist.put(neighbor, tentativeDist);
                    openSet.add(new NodeRecord(neighbor, tentativeDist));
                }
            }
        }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            AStarResult result = dijkstraSearch(graph, ambulanceId, accidentId);
